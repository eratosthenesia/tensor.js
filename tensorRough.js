...
function pr() {
  ...
  if(arguments) {
    for(var i=0;i<arguments.length;++i) {
      if(arguments[i].hasOwnProperty('inspect')) {
        r += arguments[i].inspect()+' ';
      }
      else {
        r += arguments[i]+' ';
      }
...
// Takes a tensor expression, some tensors, etc. and makes a tensor
...
function getTensorSpecs(tensorstring) {
  ...
  return {name:name,tensorFormatted:name+upper+lower,rankup:getrank(upper),rankdown:getrank(lower)};
}
function prepareTensorExpression(expr,tensorNamesOrder,n) {
  ...
  expr = expr.map(function(x) {
    if(!inSorted(x,sortedOps)) {
      ...
      return {name:x[0],indices:x.slice(1).join('').split(''),specs:specs};
    } else {
      return x;
    }
  });
  ...
  expr[0] = (function(indexPicker) {
    return {specs:expr[0].specs,func:function(data,indices,exprValue) {
...
prepareTensorExpression('C^i_k := A^i_j * B^j_k - 1',['A','B'],4)
...
function numArgOp(num,arg,op,argfunc) {
  argfunc = argfunc === undefined ? idFunc : argfunc;
  return function(){
    return op(num,)
  }
}
function mapBottom(xss, func, atBottom) {
  atBottom = atBottom === undefined ? function(x){return typeof x !== 'object';} : atBottom;
...
function joinFancy(xs,joiners) {
  ...
  return intersplice(xs.map(function(x){return joinFancy(x,joiners.slice(1));}),joiners[0]);
//  joinFlatter(xs.map(function(x){return joinFancy(x,joiners.slice(1));}),joiners[0]);
}

function splitStringHelper(str,joiners) {
  if(typeof str === 'object') {
    return intersplice(str.map(function(x){return joinFancy(x,joiners.slice(1));}),joiners[0]);
  }
  return str;
}
function splitString(str) {
  ...
  for(var i=0;i<splits.length;++i) {
    r = mapBottom(r,function(s){return s.split(splits[i]);});
  }
...
splitString('abacus','b','c')
function intersplice(xs,y) {
  return joinFlatter(xs.map(function(x){return [x];}),y);
}
function joinFlatter(xss,glue) {
  var xs = new Array(xss.length-1+xss.map(function(x){return x.length;}).reduce(plusOp));
  var j=0;
  for(var i=0;i<xss.length;++i) {
...
function getNthArger(x){return function(){return arguments[x];};}
function valueFunc(x){return function(){return x;}}
function idFunc(x) {return x;}
function minusOp(a,b) {return b - a;}
function plusOp(a,b) {return b + a;}
function timesOp(a,b) {return b * a;}
...
function zipLists() {
  ...
  var n = Math.min.apply(this,lists.map(function(x){return x.length;}));
  var r = new Array(n);
  for(var i=0;i<n;++i) {
    r[i] = lists.map(function(x){return x[i];});
  }
  return r;
}
...
function simpleRPNToFunction(rpn,variables) {
  var i = 0;
  var stack = [];
  var variableIndices = numberList(variables).sort(function(x,y) {return x[0]>y[0];});
  var opDict = {'-': minusOp, '+': plusOp, '*': timesOp};
...
function funcRPNToFunction(funcRPN) {
  var functionIndices = indicesWhereTrue(funcRPN,function(x){return typeof x === 'function';});
  var functions = makePicker(functionIndices)(funcRPN);
  var i = 0;
  var rpnModified = mapAtIndices(funcRPN,functionIndices,function(func) {
    return (i++)+'';
  });
  var mainFunction = simpleRPNToFunction(rpnModified,range(0,i-1).map(function(x){return x+'';}));
  return function() {
...
function JCBError(message) {
  ...
  this.talk = function(){console.log(this.message);};
}
...
inherit = inherit.bind({defaults:{override:true}});
inherit(x,y);
...
minusOrdset([1,2,4,6,7],[2,3,4,5,8])
...
function TensorChain(tensor,name,chain) {
  ...
  this.eq = function(expr,n,context) {
    ...
    var tensorNamesOrder = this.chain.map(function(x){return x.name;});
...
function Tensor(data, rankup, rankdown, n, customTensorLookup, name) {
  ...
  this.getElement = function() {
    ...
    return this.getElementFunction(context.length === 0 ? this.data : {data:this.data,context:this.context}, indices);
  }
  this.inspect = function() {
    var r = 'Tensor<'+this.rankup+','+this.rankdown+'>:'+this.n;
    return r;
  }
  logndatalen = logab(n,flatten(data).length);
...
x=new Tensor([1,0,0],1,0,3);
y=new Tensor([0,1,0],1,0,3);
x.m('X').m(y,'Y').m(leviCivitaTensor().staticCopy(3),'e').eq('C^i:=e_ijk*X^j*Y_k').data